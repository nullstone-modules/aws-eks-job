...
//   -> Don't add secret for `{{ secret(...) }}` -- these are secrets that already exist in AWS
// 2. Add app access to AWS Secrets Manager secrets (var.secrets, local.capabilities.secrets)
// 3. Add k8s secret referencing associated AWS Secrets Manager secrets via the CSI driver
// 4. Add env var to job pod referencing the synced k8s secret

variable "env_vars" {
  type        = map(string)
  default     = {}
  description = <<EOF
The environment variables to inject into the job.
These are typically used to configure the job per environment.
...
variable "secrets" {
  ...
  sensitive   = true
  description = <<EOF
The sensitive environment variables to inject into the job.
These are typically used to configure the job per environment.
EOF
}
